...
def main(args):
    ...
    for step in pbar:
        ...
        if (step + 1) % config['log_freq'] == 0:
            ...
            # Log learning rates
            tb_writer.add_scalar('Phase1/q_lr', q_opt.param_groups[0]['lr'], global_step)
            tb_writer.add_scalar('Phase1/v_lr', v_opt.param_groups[0]['lr'], global_step)
    ...
    for step in pbar:
        ...
        if (step + 1) % config['log_freq'] == 0:
            ...
            tb_writer.add_scalar('Phase2/flow_lr', flow_opt.param_groups[0]['lr'], global_step)
