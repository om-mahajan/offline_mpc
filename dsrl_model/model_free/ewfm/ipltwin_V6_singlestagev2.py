...
def ipl_preference_loss(q_critic, flow_model, batch_seg, gamma, chi2_coeff, target_clip, act_dim):
    """Optimized IPL preference loss with AMP"""
    ...
    with torch.no_grad():
        B2 = next_obs_all.shape[0]
        t = torch.ones(B2, device=next_obs_all.device)
        flow_model.condition = next_obs_all
        next_actions = flow_model(torch.zeros(B2, act_dim, device=next_obs_all.device), t)
        flow_model.condition = None
        q_next = q_critic.get_target_q(next_obs_all, next_actions).squeeze(-1)
        if target_clip:
            q_lim = 5.0 / (chi2_coeff * (gamma + 1e-6))
            q_next = q_next.clamp(-q_lim, q_lim)

    reward = qs - gamma * q_next
    ...
    return pref_loss + chi2_loss, r_uni.mean(), r_neg.mean()
...
def train_flow_step_segment(flow_model, flow_optimizer, scaler, q_critic, v_model,
    ...
    """
    Optimized segment-based flow matching with AMP and pre-allocated buffers.
    Returns tensors to avoid GPU sync in hot loop.
    """
    flow_model.train()
    ...
    # Use pre-allocated noise and time buffers
    noise = buffers.noise[:N, :act_dim]
    noise.normal_()
    t = buffers.t[:N]
    t.uniform_(1e-6, 1.0 - 1e-6)
    
    # OT interpolation with JIT
    ...
    with torch.cuda.amp.autocast():
        ...
        loss = compute_weighted_loss_jit(v_theta, u_t, w_seg, B, H)
    ...
    flow_model.condition = None
    return loss, A_seg.mean(), A_seg.std(), w_seg.max()
...
def main(args):
    ...
    for step in pbar:
        ...
        if not in_warmup:
            q_loss, r_union_mean, r_neg_mean = ipl_preference_loss(
        ...
        if (step + 1) % config['log_freq'] == 0:
            ...
            pbar.set_description(
                ...
                f"Flow={flow_loss_avg:.6f} | V={v_loss_avg:.4f}"
            )
