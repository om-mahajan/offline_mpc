...
def ipl_preference_loss(q_critic, flow_model, batch_seg, gamma, chi2_coeff, target_clip, act_dim, 
    ...
    with torch.no_grad():
        ...
        if target_clip:
            q_lim = 5.0 / (chi2_coeff * (gamma + 1e-6))
...
def train_flow_step_segment(flow_model, flow_optimizer, scaler, q_critic, v_model,
    ...
    # During warmup: only FM loss (fast, no sampling needed)
    if in_warmup:
        ...
        scaler.update()
        flow_model.condition = None
        return fm_loss, A_seg.mean(), A_seg.std(), w_seg.max(), torch.tensor(0.0, device=device)
    
    # ========== SAC-style Policy Loss (after warmup) ==========
    diffusion_steps = config.get('diffusion_steps', 1)
    
    # Fast approximate sampling for training efficiency
    with torch.no_grad():
        sampled_actions, logp = flow_model.sample_and_logprob_fast(
            s_flat, diffusion_steps=diffusion_steps
        )
    
    # Get Q-values for sampled actions
    with torch.cuda.amp.autocast():
        q_sampled = q_critic(s_flat, sampled_actions).squeeze(-1)
    
    # SAC policy loss: α * log π(a|s) - Q(s, a)
    policy_loss_per_sample = alpha * logp - q_sampled
    policy_loss_seg = policy_loss_per_sample.view(B, H)
    weighted_policy_loss = (w_seg.unsqueeze(1) * policy_loss_seg).sum() / H
    
    # Combined loss
    lambda_fm = config.get('lambda_reg', 0.1)
    total_loss = weighted_policy_loss + lambda_fm * fm_loss
    ...
    flow_model.condition = None
    return total_loss, A_seg.mean(), A_seg.std(), w_seg.max(), logp.mean()
...
def main(args):
    ...
    log_alpha = torch.tensor(np.log(config['alpha']), requires_grad=True, device=device, dtype=torch.float32)
    alpha_opt = Adam([log_alpha], lr=config.get('alpha_lr', 3e-4))
    ...
    acc_logp = torch.zeros(1, device=device)
    acc_alpha_loss = torch.zeros(1, device=device)
    acc_count = 0
    ...
    for step in pbar:
        ...
        alpha_val = log_alpha.exp().detach() if not in_warmup else config['alpha']
        
        flow_loss, A_seg_mean, A_seg_std, w_seg_max, logp_flow = train_flow_step_segment(
        ...
        if not in_warmup:
            acc_logp += logp_flow.detach()
        acc_count += 1
        
        # ========== Alpha Auto-Tuning (after warmup) ==========
        if (not in_warmup) and auto_tune_alpha:
            # logp_flow is detached, so we compute alpha_loss differently
            # α_loss = -α * (H(π) - H_target) = -α * (-logp - H_target)
            alpha_loss = -(log_alpha.exp() * (logp_flow.detach() + target_entropy))
            alpha_opt.zero_grad()
            alpha_loss.backward()
            alpha_opt.step()
            acc_alpha_loss += alpha_loss.detach()
        ...
        if (step + 1) % config['log_freq'] == 0:
            ...
            logp_avg = (acc_logp / n).item()
            alpha_loss_avg = (acc_alpha_loss / n).item() if not in_warmup and auto_tune_alpha else 0.0
            ...
            logger.log(
                ...
                f"α={alpha_val:.4f} logp={logp_avg:.3f} | "
                f"time={elapsed:.1f}s"
            )
            ...
            tb_writer.add_scalar('SAC/logp_mean', logp_avg, global_step)
            tb_writer.add_scalar('SAC/alpha_loss', alpha_loss_avg, global_step)
            tb_writer.add_scalar('SAC/entropy', -logp_avg, global_step)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--experiment", type=str, default="ipl_singlestagev2_smooth")
    ...
    parser.add_argument("--warmup_iterations", type=int, default=200000)
    ...
    parser.add_argument("--diffusion_steps", type=int, default=10, help="ODE steps for flow sampling")
