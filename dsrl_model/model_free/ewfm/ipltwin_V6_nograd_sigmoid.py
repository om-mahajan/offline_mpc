...
default_cfg = {
    ...
    "preference_iterations": int(3000),
    "q_lr": 3e-4,
    "v_lr": 3e-4,
    "q_hidden": 256,
    "v_hidden": 256,
...
def train_flow_matching_step(flow_model, flow_optimizer, q_critic,
    ...
    # --- Fixed guidance (stationary) ---
    with torch.no_grad():
        energy = q_critic(states, actions).squeeze(-1)
        energy = energy.clamp(-10.0, 10.0)

        alpha = config.get("energy_alpha", 3.0)
        weights = torch.sigmoid(alpha * energy)
...
def main(args):
    ...
    if args.use_pretrained_q:
        assert args.pretrained_q_path is not None, "Must provide --pretrained_q_path"
        ckpt = torch.load(args.pretrained_q_path, map_location=device)
        q_critic.load_state_dict(ckpt)
        print(f"✅ Loaded pretrained Q from {args.pretrained_q_path}")
    ...
    if not args.use_pretrained_q:
        ...
        logger.log("skipped Q pretraining")
        
    # Phase 2: Flow Matching
    logger.log("\n" + "="*60)
    logger.log("Phase 2: Flow Matching with Advantage Guidance (TwinQ)")
    logger.log("="*60)
    flow_start_step = pref_iters if not args.use_pretrained_q else 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--experiment", type=str, default="ipl_twinq_nograd_sigmoid_no_w")
    ...
    parser.add_argument("--preference_iterations", type=int, default=6000)
    ...
    parser.add_argument("--use_pretrained_q", action="store_true", default=True)
    parser.add_argument("--pretrained_q_path", type=str, default="/home/me22b018/safe_diff/offline_mpc/logs/merged/OfflineSwimmerVelocityGymnasium-v1/IPLV6/ipl_twinq_fixed_v6/OfflineSwimmerVelocityGymnasium-v1/ipl_flow_single_step_fixed_v6_with_viz/seed-003-2026-01-04-16-27-50/torch_save/q_critic_model_842000.pt")
