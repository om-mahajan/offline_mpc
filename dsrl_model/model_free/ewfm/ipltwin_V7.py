...
def main(args):
    ...
    noise_buf = torch.empty(batch_size, act_dim, device=device)
    t_buf = torch.empty(batch_size, device=device)
    ...
    for step in pbar:
        ...
        flow_loss, adv_mean = train_flow_step(flow_model, flow_opt, q_critic, v_model, states, actions, config, scaler, noise_buf, t_buf)
