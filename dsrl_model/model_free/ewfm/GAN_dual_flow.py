...
default_cfg = {
    ...
    # Dual-Flow Composition Parameters
    "alpha": 1.0,  # Repulsion coefficient: v_policy = v_+ - alpha * v_-
    "max_velocity_norm": 10.0,  # Clip composed velocity magnitude for stability
}

# =============================================================================
# TODO: Future Enhancements for Dual-Flow Composition
# =============================================================================
# 1. Time-varying alpha schedule: --alpha_schedule for linear/cosine annealing
#    during ODE integration (smaller alpha early, larger near t→0)
# 2. Adaptive velocity clipping: scale v_policy by max(1, ||v|| / MAX_V)
#    instead of hard clipping
# 3. Separate inference script: extract dual_flow_composed_inference.py
#    for production deployment
# 4. Alpha grid search: automated ablation over alpha ∈ [0, 0.5, 1.0, 1.5, 2.0]
# =============================================================================
...
# =============================================================================
# Composed Flow Sampler: v_policy = v_+ - alpha * v_-
# =============================================================================


@partial(jax.jit, static_argnames=("num_steps", "traj_dim"))
def euler_sample_composed_flow(
    ...
    """
    Sample trajectory using composed dual-flow vector field.
    
    Policy vector field: v_policy(x, t | s) = v_+(x, t | s) - alpha * v_-(x, t | s)
    
    Interpretation:
        - v_+ pulls samples toward acceptable behavior (positive/union flow)
        - v_- pushes samples away from negative behavior (negative flow)
        - The policy follows the resultant vector field
    
    Integrates from t=1 (noise) to t=0 (data).
    
    Args:
        flow_pos: FlowTrainState for positive/union flow (frozen)
        flow_neg: FlowTrainState for negative flow (frozen)
        obs: Conditioning observation [B, obs_dim] or [obs_dim]
        rng: JAX random key for initial noise
        alpha: Repulsion coefficient (0 = pure positive, >0 = avoidance)
        num_steps: Number of Euler steps
        traj_dim: Trajectory dimension (train_horizon * act_dim)
        max_velocity_norm: Clip velocity magnitude for stability
    ...
    def euler_step(carry, t_val):
        ...
        # Compute both velocity fields (frozen, no gradients)
        v_pos = flow_pos.apply_fn(
            flow_pos.params, x_t, t, condition=obs, train=False
        )
        v_neg = flow_neg.apply_fn(
            flow_neg.params, x_t, t, condition=obs, train=False
        )
        
        # Compose: v_policy = v_+ - alpha * v_-
        v_policy = v_pos - alpha * v_neg
        
        # Clip velocity magnitude for numerical stability
        v_norm = jnp.linalg.norm(v_policy, axis=-1, keepdims=True)
        v_policy = jnp.where(
            v_norm > max_velocity_norm,
            v_policy * max_velocity_norm / (v_norm + 1e-8),
            v_policy
        )
        
        # Euler update: x_{t-dt} = x_t - dt * v (integrating backwards)
        x_next = x_t - dt * v_policy
        
        # Return diagnostics for optional monitoring
        diagnostics = {
            "v_pos_norm": jnp.mean(jnp.linalg.norm(v_pos, axis=-1)),
            "v_neg_norm": jnp.mean(jnp.linalg.norm(v_neg, axis=-1)),
            "v_policy_norm": jnp.mean(jnp.linalg.norm(v_policy, axis=-1)),
        }
        return x_next, diagnostics
    
    # Integrate from t=1 to t=0
    t_vals = jnp.linspace(1.0, dt, num_steps)
    x_final, diagnostics_stack = jax.lax.scan(euler_step, x_t, t_vals)
...
def euler_sample_composed_flow_with_diagnostics(
    flow_pos: FlowTrainState,
    flow_neg: FlowTrainState,
    obs: jnp.ndarray,
    rng: jax.random.PRNGKey,
    ...
    """
    Sample trajectory with velocity field diagnostics.
    
    Returns:
        trajectory: [B, traj_dim] or [traj_dim]
        diagnostics: dict with mean velocity norms over integration
    """
    single_obs = obs.ndim == 1
    if single_obs:
        obs = obs[None, :]
    ...
    for t_val in t_vals:
        ...
        v_pos = flow_pos.apply_fn(flow_pos.params, x_t, t, condition=obs, train=False)
        v_neg = flow_neg.apply_fn(flow_neg.params, x_t, t, condition=obs, train=False)
        v_policy = v_pos - alpha * v_neg
        ...
        v_pos_norms.append(float(jnp.mean(jnp.linalg.norm(v_pos, axis=-1))))
        v_neg_norms.append(float(jnp.mean(jnp.linalg.norm(v_neg, axis=-1))))
        v_policy_norms.append(float(jnp.mean(jnp.linalg.norm(v_policy, axis=-1))))
        
        x_t = x_t - dt * v_policy
    
    diagnostics = {
        "v_pos_norm_mean": np.mean(v_pos_norms),
        "v_neg_norm_mean": np.mean(v_neg_norms),
        "v_policy_norm_mean": np.mean(v_policy_norms),
        "v_pos_norm_final": v_pos_norms[-1],
        "v_neg_norm_final": v_neg_norms[-1],
        "v_policy_norm_final": v_policy_norms[-1],
    }
...
def make_composed_action_fn(
    ...
    """
    Create a JIT-compiled composed action function from dual flows.
    
    Policy: v_policy = v_+ - alpha * v_-
    
    Returns a function that takes (obs_flat, rng) and returns (action, new_rng).
    
    Args:
        flow_pos: Trained positive/union FlowTrainState (frozen)
        flow_neg: Trained negative FlowTrainState (frozen)
        mu_obs: Observation mean for normalization [base_obs_dim] (or None)
        std_obs: Observation std for normalization [base_obs_dim] (or None)
        traj_dim: Trajectory dimension (train_horizon * act_dim)
        act_dim: Action dimension
        alpha: Repulsion coefficient (0 = pure positive flow)
        obs_hist_len: Observation history length (for tiling normalization stats)
        num_steps: Number of Euler integration steps
        max_velocity_norm: Clip velocity for stability
    ...
    def action_fn(obs_flat: jnp.ndarray, rng: jax.random.PRNGKey):
        ...
        # Sample trajectory from composed flow
        traj = euler_sample_composed_flow(
            flow_pos,
            flow_neg,
            obs_norm,
            sample_rng,
...
def main(args):
    ...
    for step in pbar:
        ...
        if args.use_eval and (step + 1) % args.eval_freq == 0:
            alpha = config["alpha"]
            max_v_norm = config["max_velocity_norm"]
            
            # Create composed policy action function
            action_fn_composed = make_composed_action_fn(
                flow_pos, flow_neg, mu_obs, std_obs, traj_dim, act_dim,
                alpha=alpha,
                obs_hist_len=hist_len,
                num_steps=config["diffusion_steps"],
                max_velocity_norm=max_v_norm,
            )
            
            # Evaluate composed policy: v_policy = v_+ - alpha * v_-
            eval_reward, eval_cost, eval_len, rng = evaluate_policy_jax(
                env,
                action_fn_composed,
                rng,
                obs_hist_len=hist_len,
            ...
            print(
                f"\n[Eval @ {step+1}] Composed Policy (alpha={alpha:.2f}): "
                f"Reward={eval_reward:.2f}, Cost={eval_cost:.2f}, Len={eval_len}"
            )
            
            # Velocity field diagnostics
            rng, diag_rng = jax.random.split(rng)
            sample_obs = union_obs[:16, 0, :]  # Take 16 sample observations
            _, diagnostics = euler_sample_composed_flow_with_diagnostics(
                flow_pos, flow_neg, sample_obs, diag_rng,
                alpha=alpha, num_steps=config["diffusion_steps"],
                traj_dim=traj_dim, max_velocity_norm=max_v_norm,
            )
            print(
                f"  Velocity norms: ||v_+||={diagnostics['v_pos_norm_mean']:.3f}, "
                f"||v_-||={diagnostics['v_neg_norm_mean']:.3f}, "
                f"||v_policy||={diagnostics['v_policy_norm_mean']:.3f}"
            )
            
            # Alpha=0 baseline comparison (pure positive flow)
            if alpha > 0:
                action_fn_baseline = make_composed_action_fn(
                    flow_pos, flow_neg, mu_obs, std_obs, traj_dim, act_dim,
                    alpha=0.0,  # Pure positive flow
                    obs_hist_len=hist_len,
                    num_steps=config["diffusion_steps"],
                    max_velocity_norm=max_v_norm,
                )
                eval_reward_b, eval_cost_b, eval_len_b, rng = evaluate_policy_jax(
                    env, action_fn_baseline, rng,
                    obs_hist_len=hist_len, base_obs_dim=base_obs_dim,
                )
                print(
                    f"  Baseline (alpha=0): Reward={eval_reward_b:.2f}, "
                    f"Cost={eval_cost_b:.2f}, Len={eval_len_b}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_freq", type=int, default=30000)
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Repulsion coefficient for composed policy: v_policy = v_+ - alpha * v_-. "
             "alpha=0 gives pure positive flow, alpha>0 increases avoidance of negative regions."
    )
    parser.add_argument(
        "--max_velocity_norm",
        type=float,
        default=10.0,
        help="Maximum velocity norm for numerical stability in composed flow integration."
    )
