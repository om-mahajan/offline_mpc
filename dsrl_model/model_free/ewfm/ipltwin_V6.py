...
def train_flow_matching_step(flow_model, flow_optimizer, q_critic, v_model,
                             states, actions, config, use_guidance=True, compute_diagnostics=False):
    ...
    if use_guidance:
        ...
        if compute_diagnostics:
            diagnostics['energy_mean'] = energy.mean().item()
            diagnostics['energy_std'] = energy.std().item()
            diagnostics['energy_min'] = energy.min().item()
            diagnostics['energy_max'] = energy.max().item()
    ...
    if compute_diagnostics:
        with torch.no_grad():
            v_theta_norm = torch.norm(v_theta, dim=1)
            u_t_norm = torch.norm(u_t, dim=1)
            diagnostics['v_theta_norm_mean'] = v_theta_norm.mean().item()
            diagnostics['v_theta_norm_std'] = v_theta_norm.std().item()
            diagnostics['u_t_norm_mean'] = u_t_norm.mean().item()
            diagnostics['u_t_norm_std'] = u_t_norm.std().item()
            cos_sim = F.cosine_similarity(v_theta, u_t, dim=1)
            diagnostics['cos_sim_mean'] = cos_sim.mean().item()
            diagnostics['cos_sim_std'] = cos_sim.std().item()
            diagnostics['cos_sim_min'] = cos_sim.min().item()
            relative_err = torch.norm(v_theta - u_t, dim=1) / (u_t_norm + 1e-8)
            diagnostics['relative_err_mean'] = relative_err.mean().item()
            diagnostics['relative_err_std'] = relative_err.std().item()
            diagnostics['t_mean'] = t.mean().item()
            diagnostics['t_std'] = t.std().item()
            diagnostics['x_t_norm_mean'] = torch.norm(x_t, dim=1).mean().item()
            diagnostics['action_norm_mean'] = torch.norm(actions, dim=1).mean().item()
            diagnostics['state_norm_mean'] = torch.norm(states, dim=1).mean().item()
            diagnostics['loss_per_sample_mean'] = err.mean().item()
            diagnostics['loss_per_sample_std'] = err.std().item()
            diagnostics['loss_per_sample_max'] = err.max().item()
            diagnostics['loss_per_sample_min'] = err.min().item()
            diagnostics['loss_cv'] = (err.std() / (err.mean() + 1e-8)).item()
    ...
    if use_guidance and energy is not None:
        ...
        if compute_diagnostics:
            with torch.no_grad():
                diagnostics['weight_mean'] = weights.mean().item()
                diagnostics['weight_std'] = weights.std().item()
                diagnostics['weight_max'] = weights.max().item()
                diagnostics['weight_min'] = weights.min().item()
                diagnostics['weight_max_min_ratio'] = (weights.max() / (weights.min() + 1e-10)).item()
                ess = (weights.sum() ** 2) / (weights.pow(2).sum() + 1e-10)
                diagnostics['effective_sample_size'] = ess.item()
                diagnostics['ess_ratio'] = (ess / batch).item()
                weight_entropy = -(weights * torch.log(weights + 1e-10)).sum()
                diagnostics['weight_entropy'] = weight_entropy.item()
                diagnostics['weight_entropy_ratio'] = (weight_entropy / np.log(batch)).item()
                diagnostics['unweighted_loss'] = err.mean().item()
                diagnostics['weighted_loss'] = (err * weights).sum().item()
                diagnostics['weight_loss_ratio'] = diagnostics['weighted_loss'] / (diagnostics['unweighted_loss'] + 1e-8)
    else:
        weights = torch.ones(batch, device=device) / batch
    ...
    if compute_diagnostics:
        with torch.no_grad():
            total_grad_norm = sum(p.grad.data.norm(2).item() ** 2 for p in flow_model.parameters() if p.grad is not None)
            diagnostics['grad_norm_total'] = np.sqrt(total_grad_norm)
    
    clip_grad_norm_(flow_model.parameters(), config['max_grad_norm'])
    
    if compute_diagnostics:
        with torch.no_grad():
            clipped = sum(p.grad.data.norm(2).item() ** 2 for p in flow_model.parameters() if p.grad is not None)
            diagnostics['grad_norm_clipped'] = np.sqrt(clipped)
            diagnostics['grad_was_clipped'] = 1.0 if diagnostics['grad_norm_total'] > config['max_grad_norm'] else 0.0
...
def main(args):
    ...
    # Check if we should skip Phase 1 and load Q from checkpoint
    if args.ckpt_dir and os.path.exists(args.ckpt_dir):
        logger.log("\n" + "="*60)
        logger.log("Skipping Phase 1: Loading Q critic from checkpoint")
        logger.log("="*60)
        logger.log(f"📂 Loading from: {args.ckpt_dir}")
        state_dict = torch.load(args.ckpt_dir, map_location=device, weights_only=True)
        q_critic.load_state_dict(state_dict)
        q_critic.q_target.load_state_dict(q_critic.q_network.state_dict())
        logger.log("✅ Q critic loaded successfully")
        pref_iters = 0
    else:
        ...
        for step in pbar:
            # NOTE: Do NOT pass norm_fn here - data is already normalized by get_normalized_data()
            # norm_fn is only used for evaluation (raw env observations)
            batch_seg = sample_segment_batch(
                neg_data, union_data,
                segment_len=config['segment_length'],
                batch_size=config['batch_size'],
                device=device,
                norm_fn=None,  # Data already pre-normalized
    ...
    if not args.ckpt_dir and not os.path.exists(args.ckpt_dir):
    ...
    for step in pbar:
        ...
        should_log = (step + 1) % config['log_freq'] == 0
        flow_loss, diagnostics = train_flow_matching_step(
        ...
        if should_log:
            ...
            # Console - show key diagnostic metrics
            cos_sim = diagnostics.get('cos_sim_mean', 0)
            ess_ratio = diagnostics.get('ess_ratio', 1)
            grad_norm = diagnostics.get('grad_norm_total', 0)
            pbar.set_description(
                f"Phase2 | Loss={flow_loss:.4f} | CosSim={cos_sim:.3f} | "
                f"ESS={ess_ratio:.2f} | GradNorm={grad_norm:.2f}"
            )
            
            # Text log
            logger.log(
                f"Flow step {step+1}/{flow_iters} loss={flow_loss:.6f} "
                f"cos_sim={cos_sim:.4f} ess_ratio={ess_ratio:.3f} "
                f"grad_norm={grad_norm:.2f} time={elapsed:.1f}s"
            )
            
            # === TENSORBOARD: Flow Loss ===
            tb_writer.add_scalar('Phase2/flow_loss', flow_loss, global_step)
            
            # === TENSORBOARD: Flow Field Quality ===
            tb_writer.add_scalar('Phase2/FlowField/cos_sim_mean', diagnostics.get('cos_sim_mean', 0), global_step)
            tb_writer.add_scalar('Phase2/FlowField/cos_sim_std', diagnostics.get('cos_sim_std', 0), global_step)
            tb_writer.add_scalar('Phase2/FlowField/cos_sim_min', diagnostics.get('cos_sim_min', 0), global_step)
            tb_writer.add_scalar('Phase2/FlowField/relative_err_mean', diagnostics.get('relative_err_mean', 0), global_step)
            tb_writer.add_scalar('Phase2/FlowField/v_theta_norm_mean', diagnostics.get('v_theta_norm_mean', 0), global_step)
            tb_writer.add_scalar('Phase2/FlowField/u_t_norm_mean', diagnostics.get('u_t_norm_mean', 0), global_step)
            
            # === TENSORBOARD: Per-Sample Loss Statistics (oscillation diagnosis) ===
            tb_writer.add_scalar('Phase2/LossStats/loss_per_sample_mean', diagnostics.get('loss_per_sample_mean', 0), global_step)
            tb_writer.add_scalar('Phase2/LossStats/loss_per_sample_std', diagnostics.get('loss_per_sample_std', 0), global_step)
            tb_writer.add_scalar('Phase2/LossStats/loss_per_sample_max', diagnostics.get('loss_per_sample_max', 0), global_step)
            tb_writer.add_scalar('Phase2/LossStats/loss_cv', diagnostics.get('loss_cv', 0), global_step)
            
            # === TENSORBOARD: Gradient Diagnostics ===
            tb_writer.add_scalar('Phase2/Gradients/grad_norm_total', diagnostics.get('grad_norm_total', 0), global_step)
            tb_writer.add_scalar('Phase2/Gradients/grad_norm_clipped', diagnostics.get('grad_norm_clipped', 0), global_step)
            tb_writer.add_scalar('Phase2/Gradients/grad_was_clipped', diagnostics.get('grad_was_clipped', 0), global_step)
            
            # === TENSORBOARD: Energy Weighting (if guidance enabled) ===
            if config['use_guidance']:
                tb_writer.add_scalar('Phase2/Energy/energy_mean', diagnostics.get('energy_mean', 0), global_step)
                tb_writer.add_scalar('Phase2/Energy/energy_std', diagnostics.get('energy_std', 0), global_step)
                tb_writer.add_scalar('Phase2/Energy/energy_min', diagnostics.get('energy_min', 0), global_step)
                tb_writer.add_scalar('Phase2/Energy/energy_max', diagnostics.get('energy_max', 0), global_step)
                
                tb_writer.add_scalar('Phase2/Weights/weight_std', diagnostics.get('weight_std', 0), global_step)
                tb_writer.add_scalar('Phase2/Weights/weight_max', diagnostics.get('weight_max', 0), global_step)
                tb_writer.add_scalar('Phase2/Weights/weight_max_min_ratio', diagnostics.get('weight_max_min_ratio', 1), global_step)
                tb_writer.add_scalar('Phase2/Weights/effective_sample_size', diagnostics.get('effective_sample_size', 0), global_step)
                tb_writer.add_scalar('Phase2/Weights/ess_ratio', diagnostics.get('ess_ratio', 1), global_step)
                tb_writer.add_scalar('Phase2/Weights/weight_entropy_ratio', diagnostics.get('weight_entropy_ratio', 1), global_step)
                tb_writer.add_scalar('Phase2/Weights/weighted_vs_unweighted_loss', diagnostics.get('weight_loss_ratio', 1), global_step)
            
            # === TENSORBOARD: Input Statistics ===
            tb_writer.add_scalar('Phase2/Inputs/action_norm_mean', diagnostics.get('action_norm_mean', 0), global_step)
            tb_writer.add_scalar('Phase2/Inputs/state_norm_mean', diagnostics.get('state_norm_mean', 0), global_step)
            tb_writer.add_scalar('Phase2/Inputs/x_t_norm_mean', diagnostics.get('x_t_norm_mean', 0), global_step)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_dir", type=str, default="/home/me22b018/safe_diff/offline_mpc/logs/merged/OfflineSwimmerVelocityGymnasium-v1/IPLV6")
    parser.add_argument("--experiment", type=str, default="ipl_twinqv6_unnormalised_Qnnonly")
