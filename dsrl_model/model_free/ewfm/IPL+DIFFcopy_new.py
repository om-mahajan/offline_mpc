...
def q_preference_step(q_model, q_optimizer, neg_obs, neg_acts, union_obs, union_acts, config):
    ...
    q_union_flat = q_model(torch.cat([flat_union_obs, flat_union_acts], dim=1))
    q_neg_flat = q_model(torch.cat([flat_neg_obs, flat_neg_acts], dim=1))
...
def v_update_step(q_model, v_model, v_optimizer, union_obs, union_acts, neg_obs, neg_acts, config):
    ...
    # Compute targets
    with torch.no_grad():
        q_flat = q_model(torch.cat([flat_obs, flat_acts], dim=1))
...
def compute_weights_from_qv(q_model, v_model, union_obs, union_acts, config):
    ...
    Returns:
        weights: [horizon, batch] - energy values (log-space advantages)
    """
    horizon, batch = union_obs.shape[0], union_obs.shape[1]
    device = union_obs.device
    ...
    q_flat = q_model(flat_inputs).reshape(horizon, batch)
    v_flat = v_model(flat_obs).reshape(horizon, batch)

    # Compute advantages
    #adv = q_flat - v_flat
    
    # Option 1: Use Q-V advantages (recommended)
    adv = q_flat - v_flat
    
    # Option 2: Use Q values only (your commented line)
    #adv = q_flat
    
    # Center advantages for numerical stability
    adv_centered = adv 
    
    # Optional: Scale by temperature before returning
...
def train_weighted_flow_step(score_model, flow_optimizer, union_obs, union_acts, weights, args, config):
    """
    Train the flow model with energy-guided loss.
    weights: [horizon, batch] - per-timestep advantages from Q-V
    """
    score_model.train()
    ...
    # union_acts: [H, B, act_dim] -> [B, H, act_dim]
    ...
    # Sample time for each sample in batch (same time for all timesteps)
    random_t = torch.rand(batch, device=args.device) * (1.0 - eps) + eps  # [B]
    
    # Flatten acts to [B, H*act_dim] for the model
    flat_acts = acts_reshaped.reshape(batch, -1)  # [B, H*act_dim]
    
    # Generate noise
    z = torch.randn_like(flat_acts)  # [B, H*act_dim]
    
    # Get diffusion parameters
    alpha_t, std = args.marginal_prob_std_fn(random_t)
    alpha_t = alpha_t[:, None]  # [B, 1]
    std = std[:, None]  # [B, 1]
    
    # Perturb the data
    perturbed_x = flat_acts * alpha_t + z * std  # [B, H*act_dim]
    
    # Get score from model
    score = score_model(perturbed_x, random_t)  # [B, H*act_dim]
    
    # Reshape for per-timestep loss computation
    # [B, H*act_dim] -> [B, H, act_dim]
    score_reshaped = score.reshape(batch, horizon, act_dim)
    z_reshaped = z.reshape(batch, horizon, act_dim)
    std_expanded = std.unsqueeze(1)  # [B, 1, 1] for broadcasting
    
    # Compute per-timestep losses
    # [B, H, act_dim] -> [B, H]
    individual_loss = torch.sum((score_reshaped * std_expanded + z_reshaped)**2, dim=2)
    
    # Apply energy-guided weighting
...
def main(args):
    ...
    # Prepare config for dsrl_dataset functions
    dataset_config = {
    ...
    # Check actual trajectory lengths to determine appropriate ep_len
    ...
    # Use max trajectory length to avoid truncation issues
    ep_len = max_traj_len
    ...
    if args.normalize_observation:
        ...
        # Convert to torch tensors for normalization function
    ...
    # PRE-LOAD DATA TO GPU for faster training (if --preload_to_gpu flag is set)
    ...
    for step in pbar:
        ...
        neg_obs, neg_acts, union_obs, union_acts, union_rew = sample_trajectory_batch_from_splits(
            ...
            data_on_gpu=args.preload_to_gpu  # NEW PARAMETER
        )
        ...
        if (step + 1) % config["log_freq"] == 0 or (step + 1) == q_pretrain_iters:
            ...
            logger.dump_tabular()
            """
            logger.log(f"Pretrain step {step+1}/{q_pretrain_iters} Qpref={q_stats['pref_loss']:.4f} Qreg={q_stats['reg_loss']:.6f} Vloss={v_stats['v_loss']:.6f} time={elapsed:.1f}s")
    ...
    # ============= PHASE 2: Train flow using Q,V-derived weights ============
...
if __name__ == "__main__":
    ...
    parser.add_argument("--normalize_observation", action="store_true", default=True)
    ...
    parser.add_argument("--energy_alpha", type=float, default=3.0,
                    help="Temperature for energy-guided loss (default: 3.0)")
    parser.add_argument("--batch_size", type=int, default=128)
    parser.add_argument("--train_horizon", type=int, default=5)
    parser.add_argument("--q_pretrain_iterations", type=int, default=100000)
    parser.add_argument("--flow_train_iterations", type=int, default=100000)
