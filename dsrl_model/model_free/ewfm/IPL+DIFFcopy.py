...
def compute_weights_from_qv(q_model, v_model, union_obs, union_acts, config):
    ...
    # Center advantages before exponentiating
    adv_centered = adv - torch.max(adv)
    
    temp = config.get("cost_weight_temp", 1.0)
    weights = torch.exp(adv_centered * temp)
...
def train_weighted_flow_step(score_model, flow_optimizer, union_obs, union_acts, weights, args, config):
    """
    Train the flow model on entire action trajectories conditioned on the first observation.
    """
    score_model.train()
    ...
    traj_acts = union_acts.permute(1, 0, 2).reshape(batch, -1)  # [B, H*act_dim]
    traj_weights = torch.exp(torch.mean(torch.log(weights + EP), dim=0))  # [B]

    score_model.condition = cond_obs.to(args.device)
    flat_acts = traj_acts.to(args.device)
    flat_weights = traj_weights.to(args.device)

    eps = 1e-3
    random_t = torch.rand(flat_acts.shape[0], device=args.device) * (1.0 - eps) + eps
...
def plot_training_logs(log_dir):
    ...
    groups = {
        ...
        "V Loss": ["V/Loss"],
        "Flow Metrics": ["Flow/Step", "Flow/Loss"],
        "Eval Metrics": ["Eval/Reward", "Eval/Cost", "Eval/Length"]
    }
